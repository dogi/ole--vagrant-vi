...
pushd "%CD%"
CD /D "%~dp0"
powershell -ExecutionPolicy bypass -Command "& {Write-Host 'Uninstalling Bonjour...'; choco uninstall bonjour -y; $vm = Read-Host 'Do you want to remove the virtual machine? (Y)es, (N)o'; $box = Read-Host 'Do you want to remove also all the files? (Y)es, (N)o'; $vb = Read-Host 'Are you sure you want to remove Virtualbox? (Y)es, (N)o'; $git = Read-Host 'Are you sure you want to remove Git? (Y)es, (N)o'; $ff = Read-Host 'Are you sure you want to remove Firefox? (Y)es, (N)o'; $ch = Read-Host 'Are you sure you want to uninstall Chocolatey? (Y)es, (N)o'; if ($vm.ToUpper() -eq 'Y' -or $vm -eq '') {C:\HashiCorp\Vagrant\bin\vagrant.exe destroy community -f} if ($box.ToUpper() -eq 'Y' -or $box -eq '') {C:\HashiCorp\Vagrant\bin\vagrant.exe box remove ole/jessie64 -f} choco uninstall vagrant -y; if ($vb.ToUpper() -eq 'Y' -or $vb -eq '') {choco uninstall virtualbox -y} if ($git.ToUpper() -eq 'Y' -or $git -eq '') {choco uninstall git, git.install -y; Remove-Item $env:ALLUSERSPROFILE\git -Force -Recurse} if ($ff.ToUpper() -eq 'Y' -or $ff -eq '') {choco uninstall firefox -y} Remove-Item C:\Users\*\Desktop\MyBeLL.lnk -Force; if ($ch.ToUpper() -eq 'Y' -or $ch -eq '') {choco uninstall chocolatey -y;exit 5}}"
if "%errorlevel%" equ "5"  (
